...
export function HomeScreen({
  ...
  const hasINR = currencyBreakdown.INR > 0;
  const hasAED = currencyBreakdown.AED > 0;
  ...
  return (
    ...
    >
      <section className="hero" aria-label="Net wealth">
        ...
        {/* Two-row dual currency display */}
        <div className="hero__values">
          {hasINR && (
            <p className="hero__value hero__value--primary">
              {formatMoney(currencyBreakdown.INR, "INR")}
            </p>
          )}
          {hasAED && (
            <p className="hero__value hero__value--secondary">
              {formatMoney(currencyBreakdown.AED, "AED")}
            </p>
          )}
          {!hasINR && !hasAED && (
